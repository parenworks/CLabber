...
*** UI & UX
- Pure ANSI terminal UI via charmed (no ncurses dependency)
- Widget-based TUI with roster, buffer bar, and split chat panes
- Mouse text selection and clickable URLs
...
clabber/
  ...
  src/
    ...
    config.lisp         ; Account & app configuration
    model.lisp          ; CLOS domain classes (buffer, contact, message)
    theme.lisp          ; Theming system (Tokyo Night default)
    widgets.lisp        ; Panel widgets — pure ANSI via charmed
    layout.lisp         ; Frame computation & split panes
    emoji.lisp          ; Emoji shortcode picker
    http-upload.lisp    ; XEP-0363 file upload
    app.lisp            ; Event loop & command dispatch
    main.lisp           ; Entry point
    xmpp/
      xml.lisp          ; XML parser & serialiser
      stream.lisp       ; XMPP stream management
      sasl.lisp         ; SASL authentication
      stanza.lisp       ; Stanza types & parsing
      connection.lisp   ; Connection lifecycle
    crypto/
      signal-ffi.lisp   ; Signal protocol FFI
      omemo.lisp        ; OMEMO encryption layer
#+end_src
