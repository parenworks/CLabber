...
(defclass participants-panel (panel)
  ((participants :initarg :participants :accessor participants-list :initform nil)
   (selected-index :initform -1 :accessor participants-selected-index
                   :documentation "Index of selected participant (-1 = none)"))
...
(defmethod panel-render ((panel participants-panel))
  (let ((x (panel-x panel))
        ...
        (h (panel-height panel))
        (theme (current-theme)))
    (let ((sel (participants-selected-index panel)))
      (draw-box x y w h :title "WHO" :active-p (>= sel 0))
      (clear-panel-content x y w h)
      (let ((row (1+ y))
            (content-w (- w 2))
          ...
          (dolist (entry sorted)
            (when (< row (+ y h -1))
              ...
              (let* ((nick (clabber.model:participant-nick entry))
                ...
                (when selected-p
                  (emit-bg (theme-border-active theme) *terminal-io*)
                  (princ (make-string content-w :initial-element #\Space) *terminal-io*)
                  (cursor-to row (+ x 1)))
                ;; Role prefix in a distinct color
                (when (> (length prefix) 0)
                  (when selected-p (emit-bg (theme-border-active theme) *terminal-io*))
                  (emit-fg (theme-system-color theme) *terminal-io*)
                  (bold)
